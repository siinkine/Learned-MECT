...
class Iteration(nn.Module):
    ...
    def forward(self, cur, y):
        ...
        # Iteration update
        return  cur + self.stepsize * dx, Ax
    

class IterativeNetwork(nn.Module):
    def __init__(self, niter, op, op_adj,init_op, loss, eta, alpha):
        super().__init__()
        self.niter = niter
        self.loss = loss
...
#        current = torch.zeros_like(true)
        ...
        for i in range(self.niter):
            ...
            current, Ax = iteration(current, y)
            
            #if writer:
              #  util.summary_image(writer, 'iteration_{}'.format(i), current, it)
        
        loss1 = self.loss(current, true)
        loss2 = self.loss(Ax, y)    
        lossReturn = loss1 + self.alpha*loss2
        
        return current, lossReturn
